...
GPCharacter::GPCharacter() : AnimatedSprite("./Ressources/Perso.PNG"), _state(STANDING_RIGHT), _movingLeft(false), _movingRight(false),
                        _jump(false), _onGround(-1), _camera(nullptr)
{
    _weName = "GPCharacter";
...
GPCharacter::~GPCharacter()
{
}


int GPCharacter::update(double seconds)
{
...
void GPCharacter::mouseMoved(Vector2d pos)
{
	_mousePos = pos;
}

void GPCharacter::keyPressed(Key::Key key)
{
	if (key == Key::A) {
		_trust = 1;
        ...
        _movingRight= true;
    } else if (key == Key::Up || key == Key::Space) {
	    if (_onGround==getPosition().y) {
	        _jump = true;
	    }
	} else if (key == Key::S) {
...
void GPCharacter::keyReleased(Key::Key key)
{
...
void GPCharacter::handleCollisionWith(WorldElement * weColided, double secsSinceLastFrame, int ...)
{
...
void GPCharacter::setCamera(Camera *camera)
{
    _camera = camera;
}
